      ...
      <div className="p-6">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2 flex-wrap">
              ...
              <div className="flex gap-0.5" aria-label={`${testimonial.rating} van 5 sterren`}>
                {[...Array(5)].map((_, i) => (
                  <svg key={i} className={`w-5 h-5 flex-shrink-0 ${i < testimonial.rating ? 'text-accent-red' : 'text-primary-dark/20'}`} fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  </svg>
                ))}
              </div>
            </div>
            {testimonial.date && <p className="text-sm text-primary-dark/50 mt-0.5">{testimonial.date}</p>}
          </div>
        </div>
        <p className="text-primary-dark text-[15px] leading-relaxed">{testimonial.text}</p>
        {(testimonial.car || testimonial.service) && (
          <div className="mt-4 pt-4 border-t border-primary-dark/5 flex items-center gap-2 flex-wrap">
            ...
            <span className="text-xs text-primary-dark/60">{[testimonial.car, testimonial.service].filter(Boolean).join(' · ')}</span>
          </div>
        )}
      </div>
    </motion.article>
